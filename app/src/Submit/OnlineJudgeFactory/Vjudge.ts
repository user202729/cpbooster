...
export default class Vjudge extends OnlineJudge {
  ...
  async uploadFile(filePath: string, page: Page, langAlias: string): Promise<boolean> {
    try {
      ...
      if (!usedLangAlias) {
        console.log(
          `None of the specified language alias (${configuredLangAliases}) can be found for this online judge (available aliases are ${availableAliases})`
        );
        return false;
      }
